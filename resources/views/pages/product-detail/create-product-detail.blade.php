...
<div class="content">
    ...
    <div style="margin-top: 50px;">
        <form action="{{ url('product-detail/create-product-detail') }}" name="myForm" method="POST">
            @csrf
        ...
        <div class="form-group">
          ...
          <input name="warehouse" type="text" class="form-control"
              id="idWarehouse" aria-describedby="emailHelp" 
              onclick="myFunctionWarehouse()"
              placeholder="Tên kho ...">
              <div class="dropdown">
                  <div id="myDropdown1" class="dropdown-content">
                      <input type="text" placeholder="Search.." id="myInput1"
                          onkeyup="filterFunctionWarehouse()">
                      @forelse ($warehouses as $itemType)
                      <a id='{{ $itemType->name }}' class="aa" onclick="choose({{ $itemType->name }})">{{ $itemType->name }}</a>
                      @empty
                      @endforelse
                  </div>
              </div>
      ...
      <div class="form-group">
        ...
        <input name="supplier" type="text" class="form-control"
            id="idSupplier" aria-describedby="emailHelp" 
            onclick="myFunctionSupplier()"
            placeholder="Tên nhà cung cấp ...">
            <div class="dropdown">
                <div id="myDropdown2" class="dropdown-content">
                    <input type="text" placeholder="Search.." id="myInput2"
                        onkeyup="filterFunctionSupplier()">
...
<script>
  function myFunctionWarehouse() {
      // document.getElementById("myDropdown").classList.toggle("show");
      $('#myDropdown1').show();
  }
  function choose(name) {
      $('#myDropdown1').hide();
      $('#idWarehouse').val(`0${name}`);
  }
  function filterFunctionWarehouse() {
